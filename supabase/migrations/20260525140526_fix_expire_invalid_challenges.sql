-- Restore expire-invalid-challenges logic that was lost when confirm_match was
-- rewritten for same-row swaps. Also updates the validity check to match the
-- new canChallenge rules (same-row: challenger.col > opponent.col).
--
-- A challenge is valid when:
--   cross-row: challenger.row = opponent.row + 1
--   same-row:  challenger.row = opponent.row AND challenger.col > opponent.col
...
begin
  ...
  if v_winner_pos.profile_id is not null and v_loser_pos.profile_id is not null then
    -- Cross-row: winner was one row below loser
    if v_winner_pos.row = v_loser_pos.row + 1 then
      v_should_swap := true;
      v_loser_leaves_top  := (v_loser_pos.row = 1);
      v_winner_reaches_top := v_loser_leaves_top;
    -- Same-row: winner was behind loser (higher col = lower rank within row)
    elsif v_winner_pos.row = v_loser_pos.row and v_winner_pos.col > v_loser_pos.col then
      v_should_swap := true;
    end if;
  end if;

  if v_should_swap then
    -- 1. DELETE winner to free their slot
    delete from ladder_positions
      where profile_id = v_match.winner_id and season_id = v_match.season_id;

    -- 2. Move loser into the now-free winner slot; bank days_on_top if loser was on top
    ...
    -- 3. Re-insert winner at loser's old slot
    ...
    -- Expire open challenges that are no longer valid after positions swapped.
    -- Valid = cross-row (challenger.row = opponent.row + 1)
    --      OR same-row forward (challenger.row = opponent.row AND challenger.col > opponent.col)
    update challenges c
    set status = 'expired', responded_at = now()
    where c.status in ('pending', 'accepted', 'countered')
      and (
        c.challenger_id in (v_match.winner_id, v_match.loser_id)
        or c.opponent_id in (v_match.winner_id, v_match.loser_id)
      )
      and not exists (
        select 1
        from ladder_positions lp_ch
        join ladder_positions lp_op
          on lp_op.profile_id = c.opponent_id
         and lp_op.season_id = v_match.season_id
        where lp_ch.profile_id = c.challenger_id
          and lp_ch.season_id = v_match.season_id
          and (
            lp_ch.row = lp_op.row + 1
            or (lp_ch.row = lp_op.row and lp_ch.col > lp_op.col)
          )
      );
  end if;
