-- Track how many days a player has been at the top of the pyramid (row = 1).
--
-- days_on_top: accumulated historical days at the top (banked when leaving row 1)
-- top_since:   timestamp when the current top-stint started (null if not on top)
-- Total days shown to user = days_on_top + floor((now - top_since) / 1 day)

set search_path = public;

alter table ladder_positions
  add column days_on_top integer not null default 0,
  add column top_since timestamptz;

-- Seed existing row-1 players with top_since = season start date.
update ladder_positions lp
set top_since = s.started_at
from seasons s
where lp.season_id = s.id and lp.row = 1;

-- Update ladder view to expose computed total_days_on_top.
create or replace view ladder
  ...
  select
    ...
    lp.last_match_at,
    lp.days_on_top +
      case when lp.top_since is not null
        then greatest(0, floor(extract(epoch from (now() - lp.top_since)) / 86400))::int
        else 0
      end as total_days_on_top
  from profiles p
  join ladder_positions lp on lp.profile_id = p.id
  where p.role = 'player';

-- Replace confirm_match to bank/assign days_on_top on position swaps.
...
begin
  ...
  if v_should_swap then
    ...
    -- If loser was on top, bank their days and clear top_since.
