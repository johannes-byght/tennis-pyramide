-- After a position swap in confirm_match, automatically expire any open challenges
-- whose participants are now too far apart in the pyramid (canChallenge = false).
--
-- canChallenge: challenger.row = opponent.row  OR  challenger.row = opponent.row + 1
-- If neither holds after the swap, the challenge is set to 'expired'.
...
begin
  ...
  if v_should_swap then
    -- Loser moves down to winner's old slot.
    ...
    -- Winner moves to loser's old slot (re-insert to move primary key).
    ...
    -- Expire open challenges that are no longer valid after this position swap.
    -- A challenge is valid when: challenger.row = opponent.row OR challenger.row = opponent.row + 1.
    -- We check every open challenge where at least one participant is the winner or loser.
    update challenges c
    ...
    where c.status in ('pending', 'accepted', 'countered')
      ...
      and not exists (
        ...
        where lp_ch.profile_id = c.challenger_id
          ...
          and (lp_ch.row = lp_op.row or lp_ch.row = lp_op.row + 1)
      );
  end if;
